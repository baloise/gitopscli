...
class GitOpsConfigLoaderTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.create_git_mock = add_patch("gitopscli.commands.common.gitops_config_loader.create_git")
        ...
        self.git_util_mock.get_full_file_path.side_effect = lambda x: f"/repo-dir/{x}"
        self.gitops_config_mock.return_value = self.gitops_config_mock

    def test_happy_flow(self):
        gitops_config = load_gitops_config(git_config=self.git_config, organisation="ORGA", repository_name="REPO")

        assert gitops_config == self.gitops_config_mock

        assert self.mock_manager.method_calls == [
            call.create_git(self.git_config, "ORGA", "REPO"),
    ...
    def test_file_not_found(self):
        ...
        with pytest.raises(GitOpsException) as ex:
            load_gitops_config(git_config=self.git_config, organisation="ORGA", repository_name="REPO")
